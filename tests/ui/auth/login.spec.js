import {
  ...
  verifyLoginPage,
} from "../../../src/pages/auth/login.page";
...
test.describe("Login functionality", () => {
  test("should login successfully with valid credentials", async ({ page }) => {
    ...
    expect.soft(checkLoginPageAvialble).toEqual("Customer Login");
    let pageStatus = await login(config.loginPage, page, USERNAME, PASSWORD);
    await expect.soft(pageStatus).toHaveTitle(/Account – Sauce Demo/);
  });
  ...
  test("should fail to login with invalid credentials", async ({ page }) => {
    ...
    expect.soft(validationMessage).toContain("@");
  });
});
